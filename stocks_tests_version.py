...
class Stock(object):
	...
	# TECH USED: ALPHA VANTAGE TIME SERIES API
	# Retrieve self.data for the given stock.  Comes in a Pandas-style self.dataframe which includes the stock's opening price, closing price,
	# and more over the past ? days.
	...
	def getMaxDropDate(self):
		# Find biggest price drop within the time frame
		smallest_gap = 0
		date_index = 0
		...
		return (date_index, data.iloc[date_index][3])
	...
	def getMaxDropCoordinates(self):
		...
		reset = self.data.reset_index()
		x1,x2 = data['date'][date_index-1], data['date'][date_index]
		y1, y2 = drop.iloc[0]['4. close'], drop.iloc[1]['4. close']

		# Find the two points.  Date strings will be used to plot on the x-axis, and prices will be used to plot on the y-axis
		# the y-axis 
		return x1, y1, x2, y2

	# TECH USED: Plotly graphing library
	...
	def getGraphURL(self):
		...
		trace1 = go.Candlestick(x=data1.date,
		                       name='Stock Price',
		                       open=data1['1. open'],
		                       high=data1['2. high'],
		                       low=data1['3. low'],
